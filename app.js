...
let customDateRange = null; // {inicio: Date, fim: Date} quando usando intervalo customizado
let chartRight;
let chartCapacidade;
...
function bindEvents() {
    ...
    elements.anoFiltro.addEventListener("change", () => { customDateRange = null; atualizarDados(); });
    elements.mesFiltro.addEventListener("change", () => { customDateRange = null; atualizarDados(); });
...
function construirConsultas() {
    // Se tem range customizado que cruza meses, gerar consulta por mês
    if (customDateRange) {
        const meses = new Map();
        const cursor = new Date(customDateRange.inicio.getFullYear(), customDateRange.inicio.getMonth(), 1);
        const limite = new Date(customDateRange.fim.getFullYear(), customDateRange.fim.getMonth(), 1);
        while (cursor <= limite) {
...
function obterIntervaloRecorte(anoSel, mesSel) {
    // Se tem range customizado, usa ele
    if (customDateRange) {
        return { inicio: customDateRange.inicio, fim: customDateRange.fim };
    }
